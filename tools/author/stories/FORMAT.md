...
- `effect` is one of the names in `docs/effects/effects.json` under
  ...
  `sunset`, `clouds`, `wind`, `fireflies` (outdoors packs); and `dimlight`
  (indoors packs). The pack's `setting` in its manifest decides which are
  allowed; a `none` pack allows no canvas cues. Only an intensity and a
  duration (`Ns`) may follow — no `xN`, `loop`, `hold` or colour.
