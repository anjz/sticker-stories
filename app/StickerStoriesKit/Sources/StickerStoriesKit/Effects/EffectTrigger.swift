...
public struct EffectTriggerFile: Equatable, Sendable {
    ...
    public init(schema: Int = EffectTriggerFile.supportedSchema, triggers: [EffectTrigger], warnings: [String] = []) {
        self.schema = schema
    ...
    public init(data: Data) throws {
        ...
        for (index, entry) in list.enumerated() {
            ...
            if let trigger = Self.decodeTrigger(fields, label: label, warnings: &warnings) {
    ...
    private static func decodeTrigger(_ fields: [String: Any], label: String, warnings: inout [String]) -> EffectTrigger? {
        guard let name = fields["effect"] as? String else {
            warnings.append("\(label): missing effect; skipped")
            return nil
        }
        guard let effect = EffectName(rawValue: name) else {
