...
- `effect` is one of the names in `docs/effects/effects.json` under
  ...
  `shootingstars`, `nebula` (space packs); and `bubbles` (underwater
  packs). The pack's `setting` in its manifest decides which are allowed;
  a `none` pack allows no canvas cues. Only an intensity and a duration
  (`Ns`) may follow — no `xN`, `loop`, `hold` or colour.
